"""
Plot the coverage of Tabular, MLP, and EMLP
...
avg_linear = np.mean(coverage_linear, axis=0)
std_linear = 1.96 * np.std(coverage_linear, axis=0) / np.sqrt(coverage_linear.shape[0])
...
avg_emlp = np.mean(coverage_emlp, axis=0)
std_emlp = 1.96 * np.std(coverage_emlp, axis=0) / np.sqrt(coverage_emlp.shape[0])
...
avg_mlp = np.mean(coverage_mlp, axis=0)
std_mlp = 1.96 * np.std(coverage_mlp, axis=0) / np.sqrt(coverage_mlp.shape[0])
...
plt.savefig(env+"_"+"coverage.pdf")
plt.show()
